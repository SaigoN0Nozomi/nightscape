...
public class baseunitFx {
    ...
    blueHitRocket = new Effect(20, e -> {
        color(Color.white, Color.valueOf("d8f3f4"), e.fin());

        e.scaled(12f, s -> {
            stroke(s.fout() * 2);
            Lines.square(e.x, e.y, 24 * e.fin(), 45);
        });
    ...
    diluvioHitSmall = new Effect(30f, 50f, e -> {
        e.scaled(4f, b -> {
            color(Color.valueOf("d8f3f4"), b.fout());
            Fill.circle(e.x, e.y, 25);
        });

        color(Color.valueOf("d8f3f4"));
        stroke(e.fout() * 2f);
        Lines.circle(e.x, e.y, 25);

        Drawf.tri(e.x, e.y, 10 * e.fout(), 10, e.rotation);
        Drawf.tri(e.x, e.y, 10 * e.fout(), 10, e.rotation + 120);
        Drawf.tri(e.x, e.y, 10 * e.fout(), 10, e.rotation + 240);

        int points = 5;
        float offset = Mathf.randomSeed(e.id, 360f);
        for(int i = 0; i < points; i++){
            float angle = i* 360f / points + offset;
            Drawf.tri(e.x + Angles.trnsx(angle, 25), e.y + Angles.trnsy(angle, 25), 4f, 10f * e.fout(), angle);
        }
        for(int i = 0; i < points; i++){
            float angle = i* 360f / points + offset;
            Drawf.tri(e.x + Angles.trnsx(angle, 25), e.y + Angles.trnsy(angle, 25), 4f, -10f * e.fout(), angle);
